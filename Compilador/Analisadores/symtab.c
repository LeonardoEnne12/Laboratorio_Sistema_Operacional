...
void printSymTab(){ // Printa a tabela se símbolos 
    ...
    int len;
    char aux[20];
    fprintf(listing,"  Nome             Escopo          TipoID         TipoDado     ParamQt    Loc   Size     instLine	 Número Linha\n");
    fprintf(listing,"--------        ------------    ------------    ------------  ---------  ----- ------   ----------	--------------------\n");
    ...
    for (i=0;i<SIZE;++i){ 
        if (hashTable[i] != NULL){ 
            ...
            while (l != NULL){ 
           	 ...
           	 len = strlen(l->typeData);     
                 strcpy(aux, l->typeData);
                for (int i = 0, posicao = 0; i < len; i++, posicao++) {
        		if (aux[posicao] == ' ') posicao++;
        		aux[i] = aux[posicao];
    		}
                
                fprintf(listing,"%-14s  ",aux);
                
                fprintf(listing,"%-4d      ",l->paramQt);
                fprintf(listing,"%-4d  ",l->loc);
                fprintf(listing,"%-4d     ",l->size);
                fprintf(listing,"%-4d        ",l->instLine); 
                while (t != NULL){ 
        ...
        if (hashTable[i] != NULL){ 
            ...
            while (l != NULL){
                if(l->loc >= 0){
                    ...
                    if(l->instLine != 0) fprintf(memoryFile,"%-14d  ",l->instLine); 
                fprintf(memoryFile,"\n");
                }
                l = l->next;
            }
